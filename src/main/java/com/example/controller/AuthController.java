...
//                .orElseGet(() -> roleRepo.save(new Role(null, roleName)));
		...
		Role userRole = roleRepo.findByName("ROLE_" + roleName)
				.orElseGet(() -> roleRepo.save(new Role(null, "ROLE_" + roleName)));
		System.out.println("SCE32 : " + request.getUsername());
		User user = User.builder().username(request.getUsername())
				.password(passwordEncoder.encode(request.getPassword())).roles(Set.of(userRole)).build();
